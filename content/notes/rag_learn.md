...
- $rel_i$：第 $i$ 个结果的相关性分数（relevance）
  ...
  那么排在第 $i$ 位的结果，就先贡献一个基础收益 $rel_i$。
- $\log_2(i+1)$：位置折损项（discount）

  看几个位置的折损值：

  - 第1位：$\log_2(2)=1$
  - 第2位：$\log_2(3)\approx 1.585$
  - 第3位：$\log_2(4)=2$
  - 第4位：$\log_2(5)\approx 2.322$

  因为分母越来越大，所以同样的相关性分数，排得越后，贡献越小。
...
前面是用`rel_i`,现在改成：`2^{rel_i}-1`,把相关性“增益非线性放大”。> “非常相关”比“一般相关”重要得多，不只是线性多一点。
...
- 如果你的相关性就是简单的 0/1（相关/不相关），两者差别不大
- 如果相关性有多等级（0/1/2/3），通常更常用 $2^{rel_i}-1$
...
它代表这个 query 在前 k 个位置上，理论上能达到的最大收益。

具体例子，假设前 5 个结果的相关性分数是：
...
- $rel=3 \Rightarrow 2^3-1=7$
- $rel=2 \Rightarrow 2^2-1=3$
- $rel=3 \Rightarrow 7$
- $rel=0 \Rightarrow 0$
- $rel=1 \Rightarrow 1$
...
$$
\frac{1}{2.585}\approx 0.387
$$

加起来：

$$
DCG@5 \approx 7+1.893+3.5+0+0.387=12.780
$$
...
$$
IDCG@5 = \sum_{i=1}^{5}\frac{2^{rel_i}-1}{\log_2(i+1)}
$$
...
$$
IDCG@5 \approx 7+4.417+1.5+0.431=13.348
$$

$$
NDCG@5=\frac{12.780}{13.348}\approx 0.958
$$

这说明排序已经非常接近理想排序，排序质量很好
