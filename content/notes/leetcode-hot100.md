...
长度为 n 的数组，答案一定只可能在 1 ~ n+1 之间。

如果 1~n 都出现了，那答案就是 n+1

只要 1~n 里少了一个，答案就是其中最小缺失的那个

尽量把数字 x 放到它应该在的位置 x-1 上。
...
为什么这里一定要用 while，不能只用 if

因为一次交换后, 当前位置上可能又来了一个新的可交换元素。
...
当 i = 1 时: nums[1] = 4，4 应该去下标 3。交换后数组变成:[3,1,-1,4]，此时 nums [1] 变成了 1, 但 1 还没到正确位置，所以还要继续交换。
因此必须反复交换到当前位置不能再交换为止，所以要用 while。

为什么要判断 nums[nums[i] - 1] != nums[i]
